...
- Показывает прозрачный лидерборд для OBS: `http://127.0.0.1:17877/overlay/leaderboard?preset=…&period=session|day|all` (та же тема, что у чата).
- Даёт локальную панель управления со статусами, монитором сообщений, вкладкой зрителей, настройками overlay и диагностикой.
...
1. В CommRelay откройте **OBS → Подключение** (Connection): слева список источников (чат, лидерборд, журнал сообщений; баннеры пока недоступны).
2. Выберите нужный источник, нажмите **Копировать URL** (Copy URL) и в OBS добавьте **Browser** (для чата и лидерборда) или Custom Browser Dock (для журнала).
3. URL чата и лидерборда уже содержат `?preset=` активного пресета; у лидерборда также есть `period` и при необходимости `layout` / `font_size_px`.
...
1. В CommRelay откройте **OBS → Подключение**, выберите **Журнал сообщений** и нажмите **Копировать URL**.
...
| **URL `?preset=`** | Каждая сцена OBS может иметь свой Browser Source: скопируйте URL в списке источников на вкладке **Подключение** — он уже содержит активный пресет. |
