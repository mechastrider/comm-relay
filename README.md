...
- Ведёт статистику зрителей (очки, сообщения, сессия/день/всё время) в локальной базе `comm-relay.db` рядом с `config.json` — отдельный сервер БД не нужен.
...
- Даёт локальную панель с рабочими местами Live, Audience, Studio и Settings: статусы, сообщения, зрители, подготовка overlay и диагностика.
...
4. Откройте **Studio**: список источников проведёт через подключение overlay, лидерборда и журнала сообщений.
...
1. В CommRelay откройте **Studio**: слева список источников (чат, лидерборд, журнал сообщений; баннеры пока недоступны).
2. Выберите нужный источник, скопируйте **Follow active preset** (Следовать за активным пресетом) и в OBS добавьте **Browser** (для чата и лидерборда) или Custom Browser Dock (для журнала).
3. Основной URL чата и лидерборда **не** содержит `?preset=` — источник обновляется при смене активного пресета. Для сцены с фиксированным видом скопируйте закреплённый URL (**Pinned preset**). У лидерборда также есть `period` и при необходимости `layout` / `font_size_px`.
4. Задайте размер под ваш макет сцены. Не добавляйте фон вручную: источники на сцене уже прозрачные.
...
1. В CommRelay откройте **Studio**, выберите **Журнал сообщений** и нажмите **Копировать URL**.
...
| **URL Follow / Pinned** | Основная кнопка копирования — **Follow active preset** (без `?preset=`). Закреплённый URL с `?preset=` фиксирует выбранный пресет для одной сцены. Старые источники с `preset` продолжают работать. |
| **Превью** | Переключатель **Чат / Лидерборд**: у лидерборда всегда примерный (вымышленный) топ-5, не живая статистика. |
...
2. Если источник закреплён (`?preset=`), обновите Browser Source в OBS: правый клик по источнику → **Refresh cache of current page**. Источник без `preset` подхватит активный пресет по WebSocket; при смене темы всё же обновите кэш, если картинка «залипла».
