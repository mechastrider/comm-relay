...
Панель показывает только сообщения: при открытии восстанавливает до 100 последних записей, затем получает новые в реальном времени. Если вы прокрутили журнал вверх, новые сообщения не сбрасывают позицию; чтобы вернуть автопрокрутку, прокрутите ленту вниз.
...
| **Theme** | **Default** — карточки с полупрозрачным фоном. **Text only** — только текст без фона. **MW5 Mercs Cockpit panel / popups** — HUD-варианты для MechWarrior 5. Beta-темы **Space Marine 2 Vox popups**, **Austere vox** и **Identity HUD** предлагают соответственно цветные компактные карточки, почти монохромную ленту и HUD без рамок с аватарами и цветовыми линиями авторов. |
...
Для игровых тем размер панели задаётся размером самого Browser Source в OBS. Разместите источник поверх нужной области сцены, и тема растянется на этот прямоугольник. Для beta-тем **Space Marine 2** начните с узкого источника примерно **500 × 700 px** в правой части сцены 1920 × 1080, затем подстройте размер и позицию под игровой HUD.
...
- **Spacing и Theme «одинаковые»**: сравнивайте при активном чате; Compact заметнее при 5+ сообщениях; Text only лучше виден на зелёном или тёмном фоне сцены; игровые темы рассчитаны на отдельный прямоугольник Browser Source поверх игрового кадра.
