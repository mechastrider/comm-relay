...
4. Откройте **Studio**: при первом заходе откроется **Добавить в OBS** с шагами Browser Source и копированием URL; затем настройте внешний вид поверхностей (чат, лидерборд, баннеры).
...
1. В CommRelay откройте **Studio**: слева список поверхностей эфира (чат, лидерборд, алерты). Кнопка **Добавить в OBS** открывает шаги Browser Source и все копируемые URL.
2. Выберите поверхность и скопируйте **Follow active preset** (Следовать за активным пресетом) из превью — или откройте **Добавить в OBS** и скопируйте URL нужного источника. В OBS добавьте **Browser** (чат, лидерборд, алерты) или Custom Browser Dock (журнал сообщений).
3. Основной URL чата, лидерборда и алертов **не** содержит `?preset=` — источник обновляется при смене активного пресета. Для сцены с фиксированным видом скопируйте закреплённый URL (**Pinned preset**) в **Добавить в OBS** или в меню **Параметры превью** (⋯) у превью. У лидерборда также есть `period` и при необходимости `layout` / `font_size_px`.
...
1. В CommRelay откройте **Studio** → **Добавить в OBS**, выберите **Журнал сообщений** и нажмите **Копировать URL**.
...
| **URL Follow / Pinned** | На выбранной поверхности — **Follow active preset** (без `?preset=`). Закреплённый URL с `?preset=` — в **Добавить в OBS** или в **Параметры превью** (⋯). Старые источники с `preset` продолжают работать. |
