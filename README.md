...
- Во вкладке **Live → Контракты** позволяет вручную объявить одну задачу для зрителей, показать её в `/overlay/alert`, выбрать победителя с обычной наградой XP или закрыть без результата.
...
- Показывает баннеры команд, наград и автоматических приветствий на отдельном OBS Browser Source: `http://127.0.0.1:17877/overlay/alert` (звук в этом источнике; для эфира включите **Control audio via OBS**).
- По команде стримера фиксирует итоги текущего стрима и показывает их на отдельном полноэкранном OBS Browser Source: `http://127.0.0.1:17877/overlay/recap`; компактная история сессий остаётся доступна в Live.
- Даёт локальную панель с рабочими местами Live, Audience, Studio и Settings: статусы, сообщения, зрители, каталоги команд, наград и приветствий, подготовка overlay и диагностика. Приветствия нового и вернувшегося зрителя выключены по умолчанию; границу «вернувшегося» задаёт подтверждённая кнопка **Новый стрим**.
...
4. Откройте **Studio**: при первом заходе откроется **Подключение к OBS** с шагами Browser Source и копированием URL; затем настройте внешний вид поверхностей (чат, лидерборд, баннеры, итоги).
...
1. В CommRelay откройте **Studio**: список поверхностей эфира (чат, лидерборд, алерты, итоги) можно свернуть до иконок на широком экране; на узком он остаётся горизонтальным. Кнопка **Подключение к OBS** открывает шаги Browser Source и все копируемые URL.
2. В режиме **Основное** выберите поверхность и нажмите **Копировать ссылку OBS** — или откройте **Подключение к OBS** и скопируйте URL нужного источника. В OBS добавьте **Browser** (чат, лидерборд, алерты, итоги) или Custom Browser Dock (журнал сообщений).
3. Основной URL чата, лидерборда, алертов и итогов **не** содержит `?preset=` — источник обновляется при смене активного пресета. Для сцены с фиксированным видом перейдите во **Все настройки** и скопируйте закреплённый URL (**Pinned preset**) в параметрах превью либо в **Подключении к OBS**. Например, итоги используют `http://127.0.0.1:17877/overlay/recap` для активного пресета и `http://127.0.0.1:17877/overlay/recap?preset=default` для закреплённого. У лидерборда также есть `period` и при необходимости `layout` / `font_size_px`.
...
5. Задайте размер под ваш макет сцены. Источник итогов растяните на весь холст (например, `1920×1080`) и расположите выше обычных источников сцены; источник алертов оставьте отдельным. Не добавляйте фон вручную: источники на сцене уже прозрачные.
6. Оставьте CommRelay запущенным во время стрима.

Если вы поменяли порт в настройках, обновите URL в OBS.

## Итоги стрима и история сессий

В **Live** нажмите **Итоги**, проверьте сводку текущего стрима и подтвердите **Зафиксировать и показать**. Первое подтверждение навсегда сохраняет снимок этой сессии: последующие сообщения, XP и достижения продолжают учитываться в текущей сессии, но уже зафиксированный экран итогов не обновляется и при повторном показе остаётся тем же.

Показ, скрытие и повторный показ итогов не завершают стрим, не создают новую сессию и не сбрасывают счётчики. Для новой сессии по-прежнему нужна отдельная подтверждённая команда **Новый стрим**. Во вкладке **История** диалога итогов можно открыть компактные сводки текущей и предыдущих сессий, в том числе сессий без зафиксированного экрана; показать старую сессию в эфире из истории нельзя.

Источник `/overlay/recap` использует оформление выбранного активного или закреплённого пресета, а служебные подписи — язык приложения. Пока итоги скрыты, источник прозрачен. Если Browser Source кратковременно потерял соединение, он восстановит видимый неизменяемый снимок после переподключения. После полного перезапуска CommRelay видимость намеренно сбрасывается: снимок остаётся в истории, но для эфира нужно снова нажать **Показать итоги**.
...
Над журналом закреплена панель лидерборда: она показывает эфирное состояние и обратный отсчёт, а также позволяет сменить активный пресет. В режиме **Всегда виден** на ней остаётся один переключатель **Показывать лидерборд**. В режимах **Автоматически** и **По запросу** доступны **Показать на N с**, переключатель **Закрепить** и **Скрыть**; при закреплении временный показ недоступен, но скрыть лидерборд можно сразу. Сам журнал восстанавливает до 100 последних записей и прокручивается независимо: если вы ушли вверх, новые сообщения не сбрасывают позицию. Кнопка **Удалить** (Delete) удаляет запись из локальной истории, админки, dock и активного overlay. Кнопка **Reward** (Награда) выдаёт награду зрителю из каталога в **Audience** — как в Live.
...
| **Пресеты** (Presets) | Именованный вид для сцены или игры: тема, лимит, TTL, плотность, край текста, маркер платформы, панель, а для лидерборда — автоматический/фиксированный масштаб, заголовок, макет (`panel` / `chips`) и верхний предел мест. Старый `config.json` без пресетов превращается в пресет **Default**. |
...
| **Превью** | Список поверхностей слева переключает превью (**Чат / Лидерборд / Баннеры / Итоги**); у лидерборда всегда примерный (вымышленный) топ-5, а у итогов — изолированный пример, который не меняет эфир. Фон превью (белый, шахматка, игровой кадр, чёрный) — в меню **Параметры превью** (⋯). |

### Размер и содержимое лидерборда

В **Settings → Данные → Видимость лидерборда** выбирается глобальный режим: **Всегда виден**, **Автоматически** или **По запросу**. Для нового конфига значения по умолчанию — показ 15 секунд, кулдаун 300 секунд, напоминание об изменившемся рейтинге через 900 секунд и показ после награды/изменения топ-3. В режиме «По запросу» работают кнопки dock и команды Audience с действием **Показать лидерборд**; такая команда использует глобальную длительность и не показывает алерт. **Скрыть** в автоматическом режиме приостанавливает автопоказы на время кулдауна, а в режиме «По запросу» завершает текущий показ, не блокируя следующую команду. Старый конфиг без блока видимости остаётся в режиме «Всегда виден». Ручные состояния dock сбрасываются при перезапуске CommRelay. Видимость внутри страницы не переключает значок-глаз источника в OBS.

Для диагностики на localhost доступны `GET /api/leaderboard/visibility` и действия `POST /api/leaderboard/show`, `/hide`, `/pin`, `/resume` с JSON-телом `{}`. Они управляют эфирной страницей, но не меняют пресет Studio или видимость источника OBS.

В режиме **Автоматически** ширина Browser Source задаёт общий масштаб текста, портретов, отступов и рамки, а высота определяет, сколько целых строк рейтинга помещается. Например, источник `320×180` даст компактную композицию на несколько мест, а `640×360` — более крупную композицию и, если хватает высоты, больше строк. Половина строки и полоса прокрутки не показываются; поле **Макс. мест** остаётся верхним пределом (по умолчанию 5).

Для заголовка доступны **Из темы**, **Свой** и **Скрыть**. Свой текст использует оформление заголовка выбранной темы, а скрытый заголовок освобождает место строкам. В каждой строке главным показателем служит подписанный **XP**; число сообщений по умолчанию скрыто, его можно включить как вторичный показатель, который первым исчезает в узком источнике.

Режим **Фиксированный** сохраняет выбранный размер 12–48 px и нужен для совместимости или точной ручной компоновки. Старые URL с корректным `font_size_px=` продолжают включать фиксированный режим.

Важно различать размер окна Browser Source и трансформацию на сцене OBS. Если источник только растянуть маркерами трансформации, OBS масштабирует уже отрисованную картинку. Чтобы лидерборд заново рассчитал текст и число строк, измените **Width/Height** в свойствах Browser Source (или размеры превью в Studio), затем при необходимости подгоните трансформацию на сцене.
...
При первом запуске версии с историей сессий база обновляется автоматически. Старые события относятся к сессии только тогда, когда время позволяет определить её однозначно; поэтому в неоднозначных случаях исторические сводки могут быть неполными. Текущая статистика зрителей при этом не обнуляется.
...
- **Лидерборд не меняет число строк при растягивании на сцене**: измените Width/Height в свойствах Browser Source. Трансформация сцены масштабирует готовую картинку и не меняет размер viewport, по которому рассчитываются строки.
...
- **Источник итогов пуст**: это нормально, пока итоги скрыты. Проверьте `/overlay/recap`, размер на весь холст и порядок источников, затем откройте **Live → Итоги**. После перезапуска CommRelay нажмите **Показать итоги** снова; сохранённый снимок не пропадает.
