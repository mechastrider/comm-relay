...
Над журналом закреплена панель лидерборда: она показывает эфирное состояние и обратный отсчёт, позволяет **Показать**, **Закрепить**, вернуться в **Авто**, **Скрыть** и сменить активный пресет. Сам журнал восстанавливает до 100 последних записей и прокручивается независимо: если вы ушли вверх, новые сообщения не сбрасывают позицию. Кнопка **Удалить** (Delete) удаляет запись из локальной истории, админки, dock и активного overlay. Кнопка **Reward** (Награда) выдаёт награду зрителю из каталога в **Audience** — как в Live.
