...
Источник `/overlay/recap` прозрачен, пока итоги скрыты. Если Browser Source кратковременно потерял соединение, он восстановит видимый неизменяемый снимок после переподключения. После полного перезапуска CommRelay видимость намеренно сбрасывается: снимок остаётся в истории, но для эфира нужно снова нажать **Показать итоги**.
