...
1. В CommRelay откройте **Studio**: слева список источников (чат, лидерборд, алерты, журнал сообщений).
2. Выберите нужный источник, скопируйте **Follow active preset** (Следовать за активным пресетом) и в OBS добавьте **Browser** (для чата, лидерборда и алертов) или Custom Browser Dock (для журнала).
3. Основной URL чата, лидерборда и алертов **не** содержит `?preset=` — источник обновляется при смене активного пресета. Для сцены с фиксированным видом скопируйте закреплённый URL (**Pinned preset**). У лидерборда также есть `period` и при необходимости `layout` / `font_size_px`.
