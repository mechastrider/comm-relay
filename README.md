...
## Проверка overlay без эфира

В **Studio → Тестировать overlay** статическое превью остаётся быстрым способом сравнить тему, шрифт и фон, а тестовый режим проверяет живую последовательность сообщений, наград, лидерборда и алертов без чата и без изменения очков.

- **Стабильный test-only URL** можно один раз вставить в отдельный Browser Source OBS: он следует за активным пресетом, получает только тестовые события и никогда не показывает эфирный чат.
- **Snapshot URL текущего превью** добавляет к test-only адресу несохранённый внешний вид черновика. После изменения черновика его нужно скопировать заново; стабильный URL этого не требует.
- Кнопки **Запустить**, **Повторить** и **Сбросить** работают в одном общем локальном канале: действие из другой вкладки Studio или тестового источника OBS заменяет текущую последовательность. Studio показывает число подключённых получателей; `0` — успешный запуск без подключённого тестового источника.
- Окончательно проверьте звук и автозапуск в самом OBS: браузер Studio не обходит правила autoplay. Для старой версии CommRelay test-only адреса возвращают `404`; они не переключаются на эфирный overlay.
