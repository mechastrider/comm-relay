...
func TestConfig_WhenRecapPanelOpacityUpdated_ExpectPublicRoundTrip(t *testing.T) {
	t.Parallel()
  ...
  "overlay": {
	...
	preset := payload["overlay"].(map[string]any)["presets"].([]any)[0].(map[string]any)
	surfaces := preset["surfaces"].(map[string]any)
	require.Equal(t, float64(0), surfaces["recap"].(map[string]any)["panel_opacity"])
	require.Equal(t, 0.0, env.ConfigStore.Snapshot().Overlay.Presets[0].RecapPanelOpacity())
}

func TestConfig_WhenRecapPanelOpacityInvalid_ExpectAtomicRejection(t *testing.T) {
	t.Parallel()
  ...
  "overlay": {
    ...
    "presets": [{
      ...
      "message_ttl_seconds": 20, "font_size_px": 18, "display_mode": "normal", "theme": "default",
      "style": { "font_family": "system", "line_height": 1.35, "text_edge": "shadow", "text_edge_strength": 2, "platform_marker": "stripe", "panel_color": "#000000", "panel_opacity": 0.58, "border_width": 0, "border_color": "#ffffff", "border_radius": 8 },
      "surfaces": { "recap": { "panel_opacity": 1.2 } }
    }]
  }
}`)
	rec := httptest.NewRecorder()
	...
	require.Contains(t, rec.Body.String(), "overlay_preset_0_surfaces_recap_panel_opacity")
