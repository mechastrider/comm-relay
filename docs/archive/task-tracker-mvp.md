# Архив CommRelay Task Tracker — CR-001–CR-023

Статус: **исторический реестр MVP и раннего product polish**. Новые идеи учитываются в [`docs/interactive/backlog.md`](../interactive/backlog.md), а готовая к реализации работа — в OpenSpec. CR-023 остаётся видимым в текущем [`task-tracker.md`](../task-tracker.md), пока не разрешён OQ-002.

Всё ниже сохранено как история прежнего процесса. Правило автоматического выбора первой `todo`-задачи больше не действует.
