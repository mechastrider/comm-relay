...
- В OBS → Внешний вид: пресеты overlay для разных сцен (`/overlay?preset=…`), токены стиля (шрифт, тень/обводка, интервал, маркер платформы, фон, рамка), подсветка сообщений по списку слов и ручные иконки для выбранных ников; настройки применяются в OBS без Refresh cache.
