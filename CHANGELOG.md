...
- **Статистика зрителей и лидерборд**: рядом с `config.json` создаётся `comm-relay.db` (SQLite); очки за сообщения, сброс дня и объединение аккаунтов одного человя с разных платформ — во вкладке **Зрители**; **Новый стрим** сбрасывает счётчики сессии; в **OBS → Подключение** — третья карточка Browser Source для прозрачного лидерборда (`/overlay/leaderboard?period=session|day|all`).
