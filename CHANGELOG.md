...
- **Пресеты внешнего вида** OBS overlay и URL с `?preset=` для отдельного вида в каждой сцене. На **Подключении** — список источников (чат, лидерборд, журнал; баннеры позже); на **Внешний вид** — остров пресета, переключатель превью **Чат / Лидерборд** (у лидерборда всегда примерный топ-5), отдельный шрифт и макет рейтинга (`панель` / `чипы`).
