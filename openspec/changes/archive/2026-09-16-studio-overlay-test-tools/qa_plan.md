...
| Icon actions | Inspect and keyboard-activate contextual copy/refresh/replay controls and the preset create/rename/duplicate/delete group | Shared icon language, localized accessible name/tooltip, visible focus, stable async identity; preset actions remain visible without text or overflow substitution, deletion stays destructive and confirmed, and ambiguous actions elsewhere retain text | P0 |
| Raised buttons | Inspect shared text and icon buttons in light/dark Studio at rest, hover, active, focus, disabled, and busy states | Buttons read as raised when available and pressed while active without layout movement; tabs, navigation, selects, and choice chips retain their own styling | P0 |
| Preset toolbar responsiveness | Render Studio with one and multiple presets at wide, compact, and narrow supported widths | All four preset icon actions remain visible beside or wrapped with the preset controls; the URL group moves before actions overlap or clip | P0 |
