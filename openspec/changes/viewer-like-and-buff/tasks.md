...
- [ ] 1.1 Add migration: `commands.points` / `award_id`, expanded `action`, `progression_levels` quotas, `interaction_events.kind` `buff` plus recipient/parent columns and indexes.
- [ ] 1.2 Validate like/buff pairing on create/update; GET/POST JSON includes `action`, `points`, `award_id`; HTTP 400 field errors.
- [ ] 1.3 Additive config `buffs_per_award_per_viewer` (1) and `buff_max_unique_viewers` (5); reject negatives; public GET/update.
- [ ] 1.4 Level JSON `like_quota` / `buff_quota` 0–100; seed 1…5 on starter rows in the migration.
- [ ] 1.5 One-time social bootstrap: `viewer_like` award, `like`/`buff` commands (skip colliding triggers), achievements `cheerleader` / `chat_favorite` / `copilot`.
- [ ] 1.6 Tests: migrate from 00020 fixture; collision skip; config omit defaults; invalid action pairing.
...
- [ ] 2.1 Parse first bang token + remainder; social actions match remainder; `alert`/`show_leaderboard` extra words remain ordinary chat; typo still unique on token only.
- [ ] 2.2 Nick pipeline: session-active pool, normalize, exact, unique same-platform exact, Damerau ≤ 1 when length ≥ 4; `not_found` / `ambiguous`.
- [ ] 2.3 Like: required nick, bound award grant to recipient, giver command event, no giver XP, `missing_arg` / `self` / `quota`.
- [ ] 2.4 Buff: latest operator award (optional nick), XP + `buff` event, not a second original award id, `no_award` / `self` / `already_buffed` / `award_full` without consuming quota; 0 unique cap disables buffing.
- [ ] 2.5 Session quotas from current level; persist via events; New stream resets; process restart keeps remaining uses.
- [ ] 2.6 `command_outcome` `rejected` + `reason` / `reason_label`; overlay freeze shares cooldown flag; Info logs trigger/reason/ids only.
- [ ] 2.7 Tests for every reason, same-platform exception, concurrent cap, like journal, buff not inflating spotter count, `!gg alice` ordinary.
...
- [ ] 3.1 Command editor actions Like/Buff; award select and points; hide splash; field errors; constrained scroll.
- [ ] 3.2 Settings cap fields and level quota fields; RU/EN labels/helpers.
- [ ] 3.3 Rejected frozen chrome + reason label (уточни/clarify); overlay 5 s; hide flag shared with cooldown.
- [ ] 3.4 Russian `[Unreleased]` bullets for viewer like/buff, quotas, and caps (streamer-visible).
...
## Gate: qa
- [ ] Q.1 Execute `qa_plan.md` P0 admin + overlay/dock scenarios in Chromium; record evidence or explicit skip for OBS/packaged desktop.

## Gate: review
- [ ] R.1 Fresh diff review; CRITICAL=0; affected checks green.

## Gate: distribution-readiness
- [ ] D.1 Confirm no installer/signing/package-layout change; `go build ./...` succeeds; `openspec validate viewer-like-and-buff --strict`.
