...
The presentation controller stores `content` and a synchronized `visible` value for the current contract id. On startup it reads the durable active contract and defaults to `contract`/visible. A display action verifies the active id before changing the process-local state. New WebSocket clients receive a snapshot. The leaderboard client keeps the latest ranking in memory and renders either ranking or a compact, top-aligned contract card in the same themed root. The dock presents the ordinary leaderboard visibility actions as icon buttons and adds a visually separate two-button icon switcher for content plus a repeat-announcement icon. Show, Pin, Resume, Hide, automatic triggers, and timed expiry synchronize only visibility and preserve the selected content; the mode switcher changes only content and preserves visibility. Settlement broadcasts an inactive contract state; the untouched ordinary leaderboard policy then governs the surface again.
...
5. **Keep presentation control server-authoritative but process-local.** A dedicated `viewer_contract_state` snapshot converges the dock and leaderboard after reconnect. Content selection is not a durable preference: restart deliberately returns an active contract to the safest visible-objective default. Ordinary leaderboard visibility transitions are mirrored into the active presentation, while the separate content switcher never changes visibility or starts a timer.
