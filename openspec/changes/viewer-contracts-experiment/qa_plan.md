...
| dock presentation controls | Exercise the icon-only Show for N seconds, Pin/Resume, Hide, contract/ranking, and repeat controls plus the always-policy switch in both content modes | All applicable controls stay in one horizontal row without narrow-dock overflow; content and visibility states converge independently; localized tooltips and accessible group/pressed/busy states are correct | P0 |
...
- A follow-up 360×700 browser measurement placed all six timed-mode icons at the same 63 px top coordinate in a 34 px-high row. The always-policy variant kept its switch and three contract controls in the same row; both variants had a 340 px client/scroll width and the 360 px document did not overflow. Contract tooltips were right-anchored so their hidden or hover geometry did not widen the row.
