...
- [x] 1.1 Add the leaderboard presentation fields and presence-aware default/legacy resolution to config and public DTOs.
- [x] 1.2 Add field validation for sizing/title modes, custom title, message flag, and bounds while preserving alert and opacity semantics.
- [x] 1.3 Cover fresh defaults, legacy font/title cases, round trips, unchanged Publish, invalid values, and secret omission with focused Go tests.
...
- [x] 2.1 Introduce a pure bounded fit calculation and coalesced resize lifecycle with deterministic fixed fallback and complete-row clipping.
- [x] 2.2 Refactor leaderboard markup/CSS so text, avatars, gaps, padding, borders, and chrome derive from one scale across all five themes and both layouts.
- [x] 2.3 Replace generated/duplicate headings with one theme-owned title slot implementing theme, custom, and hidden modes.
- [x] 2.4 Redesign rows around prominent labelled XP, optional secondary message count, long-name truncation, and compact suppression priority.
- [x] 2.5 Add frontend tests for scale bounds, fit thresholds/hysteresis, title resolution, query compatibility, sample isolation, and message-count suppression.
...
- [x] 3.1 Add Automatic/Fixed, From theme/Custom/Hidden, Show messages, and maximum-ranks controls with conditional accessible fields.
- [x] 3.2 Carry draft values through preview, section/theme switching, Reset-to-theme, Publish, and failed-save recovery without materializing untouched legacy defaults.
- [x] 3.3 Add localized labels/help/errors and frontend tests for normalization, conditional focus, preview URL state, and EN/RU parity.
...
- [x] 4.1 Update Russian and English OBS/Studio documentation, including title modes, XP/message hierarchy, sizing examples, and transform-vs-viewport troubleshooting.
- [x] 4.2 Refine the existing Russian `[Unreleased]` leaderboard bullet without rewriting released sections.
...
- [x] V.5 Run `openspec validate responsive-xp-leaderboard --strict` and confirm the implementation diff matches the delta specs.
